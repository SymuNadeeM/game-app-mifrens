...
const GlobalTable: React.FC = () => {
  ...
  const getAllCandidates = (): CandidateInfo[] => {
    const randomData = generateRandomData(); // Assuming this generates group data
    const combined = randomData.flatMap((group) => group.candidate_info);
  ...
  useEffect(() => {
    const combinedCandidates = getAllCandidates();
    setAll(combinedCandidates);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  return (
    <div className="my-2 h-[262px] overflow-y-scroll custom-scrollbar">
      <div className="py-2.5 px-2 rounded space-y-1.5 pb-10 summary-table-bg backdrop-blur border-[.5px] border-white/[20%] relative">
        {all.map((item, index) => {
          const isLastItem = index === all.length - 1;
          return (
            <div
              key={index}
              className={`py-1.5 px-2 flex rounded items-center justify-between gap-2.5 w-full h-11 ${
                isLastItem
                  ? "bg-[#222d42] border-[#C9D15E]/[40%] sticky bottom-0"
                  : "bg-[#293557]/[20%] border-white/[5%]"
              } backdrop-blur border-[0.5px]`}
            >
